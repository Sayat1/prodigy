...
class Prodigy_SPE_DK(torch.optim.Optimizer):
    r"""
            ...
            for p in group['params']:
                ...
                if group_lr > 0.0:
                    ...
                    if beta1 > 0:
                        ...
                        exp_avg.mul_(beta1 * d_k).add_(grad, alpha=1-beta1)

                    if not factored or len(p.shape) < 2:
                        exp_avg_sq = state['exp_avg_sq']
                        exp_avg_sq.mul_(beta2 * d_k2).addcmul_(grad, grad, value=1-beta2)
                    else:
                        exp_avg_sq_row = state["exp_avg_sq_row"]
                        exp_avg_sq_col = state["exp_avg_sq_col"]

                        grad_sq=grad.square()
                        exp_avg_sq_row.mul_(beta2 * d_k2).add_(grad_sq.mean(dim=-1), alpha=1-beta2)
                        exp_avg_sq_col.mul_(beta2 * d_k2).add_(grad_sq.mean(dim=-2), alpha=1-beta2)
